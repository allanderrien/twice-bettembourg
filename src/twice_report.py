...
Aucune dépendance externe — HTML/CSS/JS pur avec Chart.js
"""
...
def generate_html(data: dict) -> str:
    ...
    # Labels avec date complète pour l'axe
    ...
    # ── Tableau chronologique (site 0, uniquement historique + quelques heures de prévision)
    chrono_rows = ""
    if resultats:
        for h in resultats[0].get("chronologie", []):
            ...
            fc_tag = ' <span style="font-size:10px;color:#0369a1;font-weight:600;">PRÉVIS.</span>' if is_fc else ""
            chrono_rows += f"""
            <tr {row_bg}>
              <td>{t}{fc_tag}</td>
              <td>{p} mm</td>
...
<style>
  ...
  * {{ box-sizing:border-box; margin:0; padding:0; }}
  ...
  .badge {{ display:inline-block; background:var(--accent); color:white; padding:3px 10px; border-radius:20px; font-size:11px; font-weight:600; margin-bottom:6px; letter-spacing:0.5px; }}
  ...
  .legend-bar {{ background:var(--white); border-bottom:1px solid var(--border); padding:10px 48px; display:flex; gap:24px; font-size:12px; color:var(--mid); align-items:center; }}
  ...
  .cards-grid {{ display:grid; grid-template-columns:repeat(auto-fit,minmax(380px,1fr)); gap:20px; }}
...
<div class="chain">
  <span class="chain-step">🌧 Précipitations (Open-Meteo)</span><span class="chain-arrow">→</span>
  ...
  <span class="chain-step">🏭 Accessibilité des sites</span><span class="chain-arrow">→</span>
...
<div class="legend-bar">
  ...
  <span class="leg"><span class="leg-box" style="background:rgba(59,130,246,0.5);"></span>Données historiques</span>
  <span class="leg"><span class="leg-box" style="background:rgba(59,130,246,0.15);border:1px dashed #93c5fd;"></span>Prévisions</span>
  <span class="leg"><span style="color:#dc2626;font-weight:600;">│</span>&nbsp;Maintenant</span>
  <span class="leg"><span class="leg-box" style="background:#fef3c7;border:1px solid #fcd34d;"></span>Activité dégradée</span>
  <span class="leg"><span class="leg-box" style="background:#fee2e2;border:1px solid #fca5a5;"></span>Activité à l'arrêt</span>
</div>
...
// ── Plugin : ligne verticale "maintenant" + zone prévision ──
const nowLinePlugin = {{
  id: 'nowLine',
  afterDraw(chart) {{
    const ctx  = chart.ctx;
    const xPos = chart.scales.x.getPixelForValue(NOW_IDX);
    const top  = chart.chartArea.top;
    const bot  = chart.chartArea.bottom;
    const right = chart.chartArea.right;

    // Zone prévision (fond bleu très clair)
    ctx.save();
    ctx.fillStyle = 'rgba(219,234,254,0.25)';
    ctx.fillRect(xPos, top, right - xPos, bot - top);
    ctx.restore();
    ...
    ctx.fillText('▼ maintenant', xPos, top - 4);
    ctx.restore();
  }}
}};
...
const TICK_FONT = {{ family: 'DM Sans', size: 10 }};
const GRID_COLOR = '#e2e2ee';

// Axe X commun : affiche 1 tick sur 12 (toutes les 12h) avec date
const xAxis = {{
  ticks: {{
    font: TICK_FONT,
    maxTicksLimit: 18,
    maxRotation: 45,
    callback: (val, i) => (i % 12 === 0) ? LABELS[i] : ''
  }},
...
new Chart(document.getElementById('chartAlea'), {{
  data: {{
    ...
    datasets: [
      {{
        type: 'bar', label: 'Précipitations (mm/h)', data: PRECIP,
        backgroundColor: LABELS.map((_, i) =>
          i <= NOW_IDX ? 'rgba(59,130,246,0.6)' : 'rgba(59,130,246,0.2)'),
        yAxisID: 'yP', order: 2,
      }},
      {{
        type: 'line', label: 'Indice d\'aléa [0–1]', data: INDICES,
        borderColor: 'rgba(220,38,38,0.9)',
        backgroundColor: 'rgba(220,38,38,0.06)',
        borderWidth: 2, pointRadius: 0, fill: true, tension: 0.3,
        borderDash: (ctx) => ctx.dataIndex > NOW_IDX ? [4,3] : [],
        yAxisID: 'yA', order: 1,
        segment: {{ borderDash: ctx => ctx.p0DataIndex >= NOW_IDX ? [4,3] : [] }}
      }}
    ]
  }},
  options: {{
    responsive: true, maintainAspectRatio: true, animation: {{ duration: 400 }},
    plugins: {{ legend: {{ labels: {{ font: TICK_FONT, boxWidth: 12 }} }} }},
    scales: {{
      x:  xAxis,
      yP: {{ type:'linear', position:'left',  title:{{ display:true, text:'mm/h', font:{{ size:10 }} }}, grid:{{ color:GRID_COLOR }}, ticks:{{ font:TICK_FONT }} }},
      yA: {{ type:'linear', position:'right', min:0, max:1, title:{{ display:true, text:'Indice', font:{{ size:10 }} }}, grid:{{ drawOnChartArea:false }}, ticks:{{ font:TICK_FONT }} }}
    }}
  }}
}});
...
const ROUTE_COLORS = ['rgba(37,99,235,0.85)','rgba(5,150,105,0.85)','rgba(217,119,6,0.85)','rgba(168,85,247,0.85)'];
...
new Chart(document.getElementById('chartRoutes'), {{
  ...
  data: {{
    labels: LABELS.map((_, i) => i),
    datasets: ROUTE_IDS.map((rid, i) => ({{
      label: rid.replace(/_/g,' '),
      data:  SITES[0].chronologie.map(h => STATUT_VAL[h.statuts_routes[rid]] ?? 1),
  ...
  options: {{
    ...
    scales: {{
      x: xAxis,
      y: {{ min:-0.05, max:1.1, grid:{{ color:GRID_COLOR }}, ticks:{{ font:TICK_FONT, callback: v => v===1?'Normal':v===0.5?'Impacté':v===0?'Coupé':'' }} }}
    }}
  }}
}});
  ...
  new Chart(document.getElementById('chartTaux0'), {{
    ...
    data: {{
      ...
      datasets: [{{
        ...
        data:  SITES[0].chronologie.map(h => h.taux_activite * 100),
    ...
    options: {{
      ...
      scales: {{
        x: xAxis,
        y: {{ min:0, max:105, grid:{{ color:GRID_COLOR }}, ticks:{{ font:TICK_FONT, callback: v => v+'%' }} }}
      }}
    }}
  }});
}}
...
if (SITES[0]) {{
  ...
  new Chart(document.getElementById('chartPertes0'), {{
    ...
    options: {{
      ...
      scales: {{
        x: xAxis,
        y: {{ grid:{{ color:GRID_COLOR }}, ticks:{{ font:TICK_FONT, callback: v => v.toLocaleString('fr-FR')+' €' }} }}
      }}
    }}
  }});
}}
